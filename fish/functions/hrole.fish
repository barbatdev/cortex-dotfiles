# Enter a named Herdr role session. Usage: hrole <role> [path]
function hrole --description 'Enter a named Herdr role session'
    if test (count $argv) -lt 1; or test -z "$argv[1]"
        printf 'Uso: hrole <rol> [path]\n' >&2
        return 2
    end
    if not command -q herdr
        printf 'hrole: herdr is not available\n' >&2
        return 127
    end
    set -l role $argv[1]
    set -l target $PWD
    test (count $argv) -ge 2; and set target $argv[2]
    ...
    set -l suffix (string replace -ra '[^A-Za-z0-9_.-]' - "$role")
    set workspace_label "$workspace_label"-"$suffix"
    if test "$HERDR_ENV" = 1
        if not command -q python3
            printf 'hrole: python3 is not available\n' >&2
            return 127
        end
...
' "$workspace_label" 2>/dev/null)
    ...
    env CORTEX_MULTIPLEXER=herdr herdr --session "$context"-"$host"-"$workspace_label"
end
